...
DATAFILE = 'C:/Github/DataAnalystNanodegree/P3/Lesson4PS1/arachnid2.csv'
...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for l in reader:
            if l['binomialAuthority_label'] != 'NULL':
                data.append({value: l[key] for key, value in fields.items()})
...
def update_db(data, db):
    pprint.pprint(db)
    for d in data:
        db.arachnid.update({"label": d["label"]},
                            {"$set": {"classification.binomialAuthority": d["binomialAuthority"]}}, multi=True)
    pass
...
# pprint.pprint(add_field(DATAFILE,FIELDS))
