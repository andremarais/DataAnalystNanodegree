...
We should use the paired samples t-test, since we have the reading time of each subject on both the Congruent and Incongruent tests. We don't know what the population parameters are, so we can't use the normal distribution. We'll be using the two-tailed test because we're only testing for a *difference*, and not if there's a directional difference between the congruent and incongruent reading times.  </br></br>
